...
@section('title') Edit edit Information
@endsection
 @push('styles')
...
<section class="content home">
  ...
  @include('_includes/breadcrumb')
    ...
    <div class="heading">
      <h3 class="font-bold col-teal">Edit map Infomation</h3>
    ...
    <!--end user heading -->
    <!--end user heading -->
    <form action="{{ route('map-info.update',$edit->id) }}" method="post">
      @csrf @method('PUT')
          ...
          <div class="card m-t-30">
            <div class="card-body p-2">
                                                  ...
                                                  <option value="">-- select Fuel Type --</option>
                                                   @foreach($fuels as $fuel)  
                                                      <option value="{{  $fuel->id }}"
                                                        @if($fuel->id == $edit->fuel_id)selected @endif
                                                        >{{  $fuel->name }}</option>  
                                                  @endforeach   
                                      </select>
              ...
              <div class="form-group {{ $errors->has('title') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="title" value="{{ $edit->title }}" id="title">
              ...
              <div class="form-group {{ $errors->has('info') ? 'is-invalid' : '' }}">
                ...
                <textarea name="info" cols="10" rows="5" class="textarea" placeholder="Write Power info" id="info">{{ $edit->info }}</textarea>
