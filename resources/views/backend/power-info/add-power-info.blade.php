...
@section('title') Add new Power info
@endsection
 @push('styles')
...
<section class="content home">
  ...
  @include('_includes/breadcrumb')
    ...
    <div class="heading">
      <h3 class="font-bold col-teal">Add New Power info</h3>
    ...
    <!--end user heading -->
    <form action="{{ route('power-info.store') }}" method="post" enctype="multipart/form-data">
      @csrf
        ...
        <div class="col-lg-12">
          <div class="card m-t-30">
            <div class="card-body p-2">
              ...
              <div class="form-group {{ $errors->has('power_id') ? 'is-invalid' : '' }}">
                <label for="display_name">Select power plant <span class="text-danger">(Requried)</span></label> @if($id)
                <input type="hidden" name="power_id" value="{{ $id }}"> @endif
                <select id="select2" name="power_id" class="form-control" style="width: 100%" @if($id) disabled @endif> 
                                  <option value="">-- select power plant --</option>
                                   @foreach($powers as $power)  
                                      <option value="{{  $power->id }}"
                                           @if($id == $power->id) selected   @endif
                                        >{{  $power->name }}</option>  
                                  @endforeach   
                      </select>

                <span class="text-danger">{{ $errors->has('power_id') ? $errors->first('power_id') : '' }}</span>
              </div>
              ...
              <div class="form-group {{ $errors->has('map_lat') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="map_lat" value="{{old('map_lat')}}" placeholder="example:-  -24.397">
              ...
              <div class="form-group {{ $errors->has('map_lng') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="map_lng" value="{{old('map_lng')}}" placeholder="example:- 24.397">
              ...
              <div class="form-group {{ $errors->has('zoom') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="zoom" value="{{old('zoom')}}" placeholder="example:- 24.397">
              ...
              <div class="form-group {{ $errors->has('marker_lat') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="marker_lat" value="{{old('marker_lat')}}" placeholder="example:-  -24.397">
              ...
              <div class="form-group {{ $errors->has('marker_lng') ? 'is-invalid' : '' }}">
                ...
                <input type="text" class="form-control" name="marker_lng" value="{{old('marker_lng')}}" placeholder="example:- 24.397">
